 ...
 */
public class Kafka04ProducerWithPartition {
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < 10; i++) {
            // 指定发送到1号分区
            kafkaProducer.send(new ProducerRecord<>("fourth", 1, "", "atguigu" + i));
            // 线程睡眠,避免全部发送到一个分区
            Thread.sleep(2);
        }
        ...
        for (int i = 0; i < 10; i++) {
            // 根据key的hash值分配分区
            kafkaProducer.send(new ProducerRecord<>("fourth", "abc", "atguigu" + i));
            // 提供线程睡眠,避免发送到同一个分区
            Thread.sleep(2);
        }
