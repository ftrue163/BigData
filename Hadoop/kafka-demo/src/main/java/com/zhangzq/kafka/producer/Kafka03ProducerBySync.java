 ...
 */
public class Kafka03ProducerBySync {
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        // 4. 调用send方法,发送消息
        for (int i = 0; i < 10; i++) {
            ProducerRecord<String, String> myProducerRecord = new ProducerRecord<>("third", "helloworld-" + i);

            // 异步发送 默认
            // kafkaProducer.send(myProducerRecord);

            // 同步发送
            // 在调用send方法时 同时调用get()方法, 能够达到同步发送的效果，但是！！！！我们在使用kafka的过程中 绝大部分情况不会这样使用，因为Kafka是一个异步架构，
            // 在出现排查问题逻辑的时候，可能会用到。
            kafkaProducer.send(myProducerRecord).get();
        }
